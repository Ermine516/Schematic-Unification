...
## and that the original problem only has a single unification Problem
    ...
    def unify_current(self):
        ...
        if self.debug==3 or (self.count==0 and self.debug>0): 
    ...
    def print_unifier_details(self,unif_terms,unif_bindings):
        ...
        for x in self.SchematicSubstitution.mappings.keys():
            print("\t"+ x+" <== "+repr(self.SchematicSubstitution.mappings[x]))
        print()
    ...
    def computeTheUnifier(self):
        shiftc = lambda l,r: (l,self.SchematicSubstitution.increment(r.func.name,r.idx,clean=True) if type(r) is Rec else r)
        shiftu = lambda l: shiftc(None,l)[1]
        recvar = list(self.SchematicSubstitution.mappings.keys())[0]
    ...
    def add_relevent(self, subp):
        ...
        def check_for_rel(t):
            if type(t) is Var:
                ...
                for vx,gx in self.SchematicSubstitution.associated_classes[t.func.name].items():
